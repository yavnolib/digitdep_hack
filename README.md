# Сервис автоматического лотирования заявок на закупку МТР
...
2. Чтобы установить и запустить сервис из переданных докер образов введите:
   ...
   ```
   ubuntu_client:/mnt# pip install --break-system-packages gunicorn
   ```
   ```
   ubuntu_client:/mnt# cd flask-app
   ubuntu_client:/mnt/flask-app# gunicorn -w 4 -b 0.0.0.0:8080 app:app
   ```
