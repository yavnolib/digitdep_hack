...
class PostgreIface:
    ...
    def upd_buyers(self, df, known):
        ...
        code_difference = list(
            set(df.code.values.tolist()) - set(known.code.values.tolist())
        )
    ...
    def upd_buyers_mode_2(self, buyer_path):
        if (Path(buyer_path).suffix == ".xlsx") or (Path(buyer_path).suffix == ".xls"):
            ...
            raise ValueError(
                "Extension of buyer's file must be in ['.xlsx', '.xls', '.csv']"
            )
        ...
        code_difference = list(
            set(buyer_df.code.values.tolist()) - set(exist_df.code.values.tolist())
        )
        tmp_df = tmp_df[tmp_df.code.isin(code_difference)]
        print(f"table: {tmp_df.shape} WILL BE ADDED.")
        with psycopg2.connect(**self.db_params) as conn:
