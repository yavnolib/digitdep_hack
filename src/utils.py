...
def find_postal_code(address: str) -> str | None:
    ...
    if (
        (not re.search(st_pattern, address))
        and (not re.search(station_pattern_i, address))
        and (not re.search(station_pattern_ii, address))
        and (not re.search(station_pattern_iii, address))
        and (not re.search(station_pattern_iv, address))
    ):
        ...
        (re.search(st_pattern, address))
        or (re.search(station_pattern_i, address))
        or (re.search(station_pattern_ii, address))
        or (re.search(station_pattern_iii, address))
        or (re.search(station_pattern_iv, address))
    ):
        station_candidates_pattern = r"\b\d{5}\b"
        station_candidates = re.findall(station_candidates_pattern, address)
        return [candidate for candidate in candidates] + [
            candidate for candidate in station_candidates
        ], 2
...
def extract_city(address):
    city_pattern = (
        r"\b(?:г\.|город\s+)?" r"([А-ЯЁ][а-яё\-]+(?:\s+[А-ЯЁ][а-яё\-]+)*)" r"\b"
    )
...
def clear_addr(addr):
    ...
    list_r = list(
        map(
            custom_strip,
            addr.replace("город", "г.").replace(",", " ").replace(".", " ").split(),
        )
    )
    if "г" in list_r:
