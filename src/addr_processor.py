...
from src.utils import find_postal_code, finder_natasha
...
class AddrParser:
    ...
    def fetch_stations(self):
        ...
        rzd_stations = rzd_stations.drop(
            rzd_stations[rzd_stations.code == 0].index
    ...
    def fetch_post_codes(self):
        ...
        post_codes = post_codes[
            ["index", "region", "autonom", "area", "city", "city_1", "geo"]
        ]
        for col in ["region", "autonom", "area", "city", "city_1"]:
            post_codes[col] = post_codes[col].apply(
                lambda x: x.lower() if isinstance(x, str) else None
            )
        post_codes.city = post_codes.city.apply(
            lambda x: x.lower().replace("-", "").replace(" ", "")
            if x is not None
            else ""
        )
    ...
    def apply_cities_cache(self):
        ...
        self.df["geo"] = self.df.apply(
            lambda x: finder_natasha(
                x.addr, self.addr_extractor, self.cities, self.cities_set
            )
            if x.geo == "nan"
            else x.geo,
            axis=1,
    ...
    def apply_postcodes_stations_cache_and_heuristics(self):
        ...
        for df_idx, (buyer_code, addr, geo) in zip(
            self.df[self.df.geo.isna()].index.tolist(),
            self.df[self.df.geo.isna()].values,
        ):
            ...
            if (mode == 1) and (some_pattern != []):
                ...
                if post_code in self.post_codes["index"].values:
                    self.df.loc[df_idx, "geo"] = self.post_codes[
                        self.post_codes["index"] == post_code
                    ].geo.values[0]
                elif post_code in self.rzd_stations.code.values:
                    self.df.loc[df_idx, "geo"] = self.rzd_stations[
                        self.rzd_stations.code == post_code
                    ].coords.values[0]
                ...
                for some_pat in some_pattern:
                    ...
                    if station_code in self.rzd_stations.code.values:
                        self.df.loc[df_idx, "geo"] = self.rzd_stations[
                            self.rzd_stations.code == station_code
                        ].coords.values[0]
                        break
                for some_pat in some_pattern:
                ...
                ("янао" in addr.lower())
                or ("ямалоненец" in addr.lower())
                or ("ямало-ненец" in addr.lower())
            ):
                ...
                ("ханты-манс" in addr.lower())
                or ("ханты манс" in addr.lower())
                or ("хантыманс" in addr.lower())
            ):
                self.df.loc[df_idx, "geo"] = "61.003180,69.018902"
                ...
                ("спб" in addr.lower())
                or ("санкт" in addr.lower())
                or ("с-петерб" in addr.lower())
            ):
                ...
                if ("ст" in tokens) and (tokens.index("ст") + 1 < len(tokens)):
                    ...
                    if (
                        tokens[tokens.index("ст") + 1]
                        in self.rzd_stations.station_name.values
                    ):
                        station_name = tokens[tokens.index("ст") + 1]
                        self.df.loc[df_idx, "geo"] = self.rzd_stations[
                            self.rzd_stations.station_name == station_name
                        ].coords.values[0]
                    elif (tokens.index("ст") + 2 < len(tokens)) and (
