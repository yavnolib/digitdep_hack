...
class MaterialCluster:
    ...
    def __create_links_from_cell(self, cell_list: np.ndarray) -> list[tuple]:
        ...
        for i in range(len(cell_list)):
            for j in range(i + 1, len(cell_list)):
                edge_list.append(
                    (cell_list[i], cell_list[j], round(random.random(), 1))
                )
    ...
    def __graph_parse(self, path: str, cols: list[str]) -> None:
        ...
        materials_by_creditor = pd.DataFrame(
            data.groupby(["creditor"])["material"].apply(lambda x: x.unique())
        ).reset_index()
    ...
    def transform(
        self,
        data: pd.DataFrame,
        mat_col: str = "Материал",
        from_csv: bool = True,
        path: str = "data/material_cluster.csv",
    ) -> pd.DataFrame:
        if not from_csv:
        ...
        if data_cluster.cluster.isna().sum() > 0:
            print(
                f"Update the file 'Исторические данные по офертам поставщиков на лот.csv', not all materials have a cluster."
                f"{data_cluster.cluster.isna().sum()=}, all={data_cluster.shape}"
            )

        data_cluster.fillna(self.get_num_clusters() + 1.0, inplace=True)
        data_cluster.rename(
            columns={"material": mat_col, "cluster": "material_cluster"}, inplace=True
        )
